...
| Error monitoring | Sentry (client) + `_shared/sentry.ts` (Edge Functions) | Client: auto-initialized in production builds only (`src/lib/sentry.ts`), so local dev never burns quota. Edge Functions: a dependency-free envelope POST rather than the SDK, gated on `SENTRY_DSN` — no-op until set, same pattern as the Twilio secrets. |
...
| Rate limiting | Postgres-backed `check_rate_limit()` RPC | Per-key sliding window (IP or user id) shared by every public-facing Edge Function: `chat`, `recommend`, `bright-task`, `create-checkout`, `wechat-auth`. `stripe-webhook` deliberately has none — Stripe's signature verification is its guard, and a limiter could drop legitimate Stripe retries. |
| Type safety | TypeScript `strict: true` | Enabled 2026-07-20. Blast radius was 9 errors, not the feared hundreds — see `CLAUDE.md` §19 for what they were, including one real (if inert) bug the type checker caught. |
